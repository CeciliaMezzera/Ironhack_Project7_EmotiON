...
def detect_emotion(images,conf=True):
    with torch.no_grad():
        ...
        for i in range(y.size()[0]):
            ...
            result.append([f"{emotions[emotion]}{f' ({100*y[i][emotion].item():.1f}%)' if conf else ''}",emotion]) # [f"{emotions[emotion]}{f' ({100*y[i][emotion].item():.1f}%)' if conf else ''}",emotion]
            

            
            with open(f'emotext.csv', 'a') as f:
                f.write(str(emotion))
